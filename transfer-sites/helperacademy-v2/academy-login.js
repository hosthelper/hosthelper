(async()=>{
  const B=window.academyBackend;if(!B)return;const{user}=await B.ready;
  const page=(location.pathname.split('/').pop()||'login.html').toLowerCase();
  const nextRaw=new URLSearchParams(location.search).get('next')||'index.html';
  ...
  form.addEventListener('submit',async e=>{
    ...
    try{
      if(page==='login.html'){
        const{data,error}=await B.client.auth.signInWithPassword({email:d.email,password:d.password});if(error)throw error;if(!data.user)throw new Error('로그인 정보를 확인해주세요.');location.replace('./'+next);
      }else{
        if(d.password!==d.password_confirm)throw new Error('비밀번호가 서로 다릅니다.');
        const{data,error}=await B.client.auth.signUp({email:d.email,password:d.password,options:{data:{name:d.name,display_name:d.name,academy_source:true}}});if(error)throw error;
        if(data.session){location.replace('./index.html')}else{show('회원가입이 접수되었습니다. 이메일 인증이 필요한 경우 인증 후 로그인해주세요.','ok');form.reset()}
      }
