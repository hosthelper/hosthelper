...
async function renderAccount(){
  ...
  const body=`<section class="account-login-banner"><div><span>✓ 카카오 로그인 완료</span><h2>${esc(name)}님</h2><p>공실헬퍼 회원 계정이 정상 연결되었습니다.</p></div><div class="account-login-actions"><a class="btn primary" href="#register">매물 등록</a><a class="btn secondary" href="#verify">자동검증</a></div></section><div class="account-summary"><article><span>활성/최근 열람권</span><b>${orders.filter(o=>o.status==='paid').length}</b></article><article><span>저장한 공간</span><b>${saved.length}</b></article><article><span>매칭 요청</span><b>${matches.length}</b></article><article><span>내 등록매물</span><b>${properties.length}</b></article></div>
