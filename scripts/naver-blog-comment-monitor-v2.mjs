...
async function fetchText(url, accept = '*/*') {
  const response = await fetch(url, {
    headers: { 'user-agent': UA, accept }, redirect: 'follow', cache: 'no-store',
...
function uniqueLogNos(text) {
  const seen = new Set();
  const out = [];
  const escaped = BLOG_ID.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const patterns = [
    new RegExp(`https?://(?:m\\.)?blog\\.naver\\.com/${escaped}/(\\d{8,})`, 'g'),
    /[?&]logNo=(\d{8,})/g,
    /["']logNo["']\s*:\s*["']?(\d{8,})/g,
  ];
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern)) {
      if (!seen.has(match[1])) { seen.add(match[1]); out.push(match[1]); }
    }
  }
  return out;
}
...
async function discoverPosts() {
  ...
  for (const [url, accept] of sources) {
    try {
      const body = await fetchText(url, accept);
      const found = uniqueLogNos(body);
      diagnostics.push({ url, bytes: body.length, found: found.length });
      for (const id of found) if (!seen.has(id)) { seen.add(id); ids.push(id); }
    } catch (error) {
      diagnostics.push({ url, error: String(error.message || error) });
    }
    if (ids.length >= MAX_POSTS) break;
  }
  if (!ids.length) throw new Error(`NAVER_POST_DISCOVERY_EMPTY:${JSON.stringify(diagnostics)}`);
  console.log('discovery', JSON.stringify(diagnostics));
  return ids.slice(0, MAX_POSTS);
}

function parsePost(html, logNo) {
  const countMatch = html.match(/\bcommentCount=["'](\d+)["']/i)
    || html.match(/\\?"commentCount\\?"\s*:\s*(\d+)/i)
    || html.match(/\bcommentCount\s*:\s*(\d+)/i);
  if (!countMatch) throw new Error(`COMMENT_COUNT_NOT_FOUND:${logNo}`);
  const titleMatch = html.match(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']*)["']/i)
    || html.match(/<title[^>]*>([^<]+)<\/title>/i);
  return {
    logNo: String(logNo),
    title: decodeHtml(titleMatch?.[1] || `네이버 블로그 글 ${logNo}`).replace(/\s*:\s*네이버 블로그\s*$/i, ''),
    commentCount: Number(countMatch[1]),
...
async function fetchPost(logNo) {
  ...
  return parsePost(await fetchText(url, 'text/html,*/*'), logNo);
}
...
async function gh(path, init = {}) {
  ...
  const response = await fetch(`https://api.github.com/repos/${STATE_REPO}/${path}`, {
    ...
    headers: {
      ...
      'user-agent': 'hosthelper-naver-comment-monitor', ...(init.headers || {}),
    },
...
async function loadState() {
  try {
    const file = await gh(`contents/${STATE_PATH}?ref=${encodeURIComponent(STATE_BRANCH)}`);
    const state = JSON.parse(Buffer.from(String(file.content || '').replace(/\s/g, ''), 'base64').toString('utf8') || '{}');
...
async function saveState(posts, sha = '') {
  ...
  const payload = {
    ...
    content: Buffer.from(JSON.stringify(state, null, 2), 'utf8').toString('base64'), branch: STATE_BRANCH,
  };
  if (sha) payload.sha = sha;
  await gh(`contents/${STATE_PATH}`, { method: 'PUT', body: JSON.stringify(payload) });
}

async function sendKakao(text) {
  const response = await fetch(KAKAO_SEND_ENDPOINT, {
    method: 'POST', headers: { 'content-type': 'application/json', 'user-agent': 'hosthelper-naver-comment-monitor/1.0' },
...
function buildMessage(events) {
  ...
  for (const e of events) lines.push('', `글: ${e.title}`, `새 댓글: +${e.delta}개 (총 ${e.commentCount}개)`, `확인: ${e.url}`);
  return lines.join('\n').slice(0, 1900);
}
  ...
  for (const logNo of ids) {
    try {
      ...
      if (previous.initialized) {
        if (before && post.commentCount > Number(before.commentCount || 0)) events.push({ ...post, delta: post.commentCount - Number(before.commentCount || 0) });
        else if (!before && post.commentCount > 0) events.push({ ...post, delta: post.commentCount });
      }
      current[logNo] = post;
    } catch (error) { failures.push({ logNo, error: String(error.message || error) }); }
  }
  const keep = {};
  for (const id of ids) if (current[id]) keep[id] = current[id];
  if (!previous.initialized) {
    await saveState(keep, previous.sha);
    console.log(JSON.stringify({ ok: true, baselineOnly: true, posts: Object.keys(keep).length, failures }));
    return;
  }
  if (events.length) await sendKakao(buildMessage(events));
  await saveState(keep, previous.sha);
  console.log(JSON.stringify({ ok: true, baselineOnly: false, posts: Object.keys(keep).length, events: events.map(e => ({ logNo:e.logNo,title:e.title,delta:e.delta,commentCount:e.commentCount,url:e.url })), failures }));
}
