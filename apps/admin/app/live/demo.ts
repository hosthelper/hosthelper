// admin은 @hosthelper/shared에 의존하지 않으므로 타입·데모 생성기를 로컬로 둔다.
...
export function initialKpi(): KpiSnapshot {
  return { at: new Date().toISOString(), activeJobs, pendingOffers, todayBookings, todayGmv: gmv, openDisputes: 1 };
}
...
const generators: Array<() => PlatformEvent> = [
  ...
  () => {
    ...
    return {
      ...
      data: { jobId: rid(), roundIdx, cleanerCount, expiresAt: new Date(Date.now() + 15 * 60_000).toISOString() },
    };
  },
  () => ({
    id: rid(),
...
export function startDemoStream({ onSnapshot, onEvent }: DemoHandlers): () => void {
  ...
  for (let i = 0; i < 6; i += 1) onEvent(makeEvent());

  const eventTimer = setInterval(() => onEvent(makeEvent()), 2200);
