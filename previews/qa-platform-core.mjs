...
const files=['universe.html','gongsil.html','academy.html','house.html','host.html','life.html','next.html','shared.js','platform-core.js','platform-data.js','platform-extras.js','platform-gongsil-policy.js','platform-mode-router.js'];
const fail=[];
...
if(/service_role|sb_secret|SUPABASE_SERVICE_ROLE/i.test(core+data+extras+gongsilPolicy+modeRouter))fail.push('server secret-like token reference found in browser code');
if(fail.length){console.error('\nPLATFORM QA FAILED\n- '+fail.join('\n- '));process.exit(1)}
console.log(`PLATFORM QA PASS: ${files.length} assets, real auth/RBAC, post-login role routing, 6 service CRUD flows, Helper ID, role approvals, supplier workspace, Kakao gate, responsive loaders.`);