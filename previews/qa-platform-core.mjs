...
const files=['universe.html','gongsil.html','academy.html','house.html','host.html','life.html','next.html','shared.js','platform-core.js','platform-data.js','platform-extras.js','platform-gongsil-policy.js'];
const fail=[];
for(const f of files){const p=root+f;if(!fs.existsSync(p))fail.push(`missing ${f}`)}
for(const f of ['shared.js','platform-core.js','platform-data.js','platform-extras.js','platform-gongsil-policy.js']){try{execFileSync(process.execPath,['--check',root+f],{stdio:'pipe'})}catch(e){fail.push(`syntax ${f}: ${e.stderr?.toString()||e.message}`)}}
...
if(/service_role|sb_secret|SUPABASE_SERVICE_ROLE/i.test(core+data+extras+gongsilPolicy))fail.push('server secret-like token reference found in browser code');
if(fail.length){console.error('\nPLATFORM QA FAILED\n- '+fail.join('\n- '));process.exit(1)}
console.log(`PLATFORM QA PASS: ${files.length} assets, real auth/RBAC, 6 service CRUD flows, Helper ID, role approvals, supplier workspace, Kakao gate, responsive loaders.`);