...
const ASSET_VERSION = process.env.HELPER_OFFICE_ASSET_VERSION || '20260912-p2-2-p1-hardening';
...
export function validateProductionBundle(files) {
  const required = ['index.html','app.js','styles.css','enhancements.js','runtime-status-v3.js','runtime-bridge-v1.js','session-recovery-v1.js','p0-hardening-v1.js','decision-rights-v1.js','customer360-v1.js','operations-v1.js'];
...
async function verifyOfficialProduction(files) {
  ...
  const markers=[`app.js?v=${ASSET_VERSION}`,`enhancements.js?v=${ASSET_VERSION}`,`runtime-status-v3.js?v=${ASSET_VERSION}`,`runtime-bridge-v1.js?v=${ASSET_VERSION}`,`session-recovery-v1.js?v=${ASSET_VERSION}`,`p0-hardening-v1.js?v=${ASSET_VERSION}`,`decision-rights-v1.js?v=${ASSET_VERSION}`,`customer360-v1.js?v=${ASSET_VERSION}`,`operations-v1.js?v=${ASSET_VERSION}`,SUPABASE_CDN];
  ...
  const[runtime,bridge,recovery,p0,rights,customer360,operations]=await Promise.all([
...
export async function deployHelperOfficeProduction() {
  ...
  const[index,app,styles,enhancements,runtimeStatus,runtimeBridge,sessionRecovery,p0Hardening,decisionRights,customer360,operations]=await Promise.all([
    githubFile('helper-office-hq/index.html',sourceCommit),githubFile('helper-office-hq/app.js',sourceCommit),githubFile('helper-office-hq/styles.css',sourceCommit),githubFile('helper-office-hq/enhancements.js',sourceCommit),githubFile('helper-office-hq/runtime-status-v3.js',sourceCommit),githubFile('helper-office-hq/runtime-bridge-v1.js',sourceCommit),githubFile('helper-office-hq/session-recovery-v1.js',sourceCommit),githubFile('helper-office-hq/p0-hardening-v1.js',sourceCommit),githubFile('helper-office-hq/decision-rights-v1.js',sourceCommit),githubFile('helper-office-hq/customer360-v1.js',sourceCommit),githubFile('helper-office-hq/operations-v1.js',sourceCommit)
  ]);
  const files=[{file:'index.html',data:prepareIndex(index)},{file:'app.js',data:app},{file:'styles.css',data:styles},{file:'enhancements.js',data:enhancements},{file:'runtime-status-v3.js',data:runtimeStatus},{file:'runtime-bridge-v1.js',data:runtimeBridge},{file:'session-recovery-v1.js',data:sessionRecovery},{file:'p0-hardening-v1.js',data:p0Hardening},{file:'decision-rights-v1.js',data:decisionRights},{file:'customer360-v1.js',data:customer360},{file:'operations-v1.js',data:operations}];
  validateProductionBundle(files);
  const created=await vercel('/v13/deployments?forceNew=1&skipAutoDetectionConfirmation=1',{method:'POST',body:JSON.stringify({name:'helper-office-hq',project:PROJECT_ID,target:'production',files,projectSettings:{framework:null},meta:{helperOfficeReleaseRef:RELEASE_REF,helperOfficeSourceCommit:sourceCommit,helperOfficeRuntimeOverlay:'p2-v2-p1-hardening',helperOfficeAssetVersion:ASSET_VERSION,source:'render-worker'}})});
