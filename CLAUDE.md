...
## 8. 상황판 (비개발자 CEO 공유 규칙)

- 사용자는 비개발자다. 설명은 용어 풀이와 함께, 클릭 단위로.
- **세션 시작 시** `docs/상황판.md`를 읽고 "지난번 이후 바뀐 것"을 3줄 이내로 먼저 공유한다.
- **의미 있는 작업 완료 시** `docs/상황판.md`의 해당 표와 갱신 로그를 업데이트하고 커밋에 포함한다.
- 상황판 구성: 코드 라이브러리 / 사업 문서 / 사업 트랙 / 다음 우선순위. 형식 유지.
