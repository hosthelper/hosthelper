...
이 저장소는 **헬퍼유니버스** 그룹의 사업 중 하나입니다. 브랜드는 9개, **법인은 호스트헬퍼 단일 사업자 하나**입니다.

| 브랜드 | 무엇을 | 비고 |
|---|---|---|
| **헬퍼유니버스** | 그룹 우산 브랜드 | 개별 상품 아님 |
| **호스트헬퍼** | 숙소 청소·턴오버 매칭 SaaS (₩10,000/건) | **이 저장소**. 사업자명이기도 함 |
| **하우스헬퍼** | 호스트 전용 쇼핑몰 | 청소 매칭이 아님 — 혼동 주의 |
| **헬퍼아카데미** | 호스트·예비창업자 교육/강의 | |
| **공실헬퍼** | 숙박 매물 양수도·권리금 진단·공인중개사 연결 | `apps/changup-site` |
| **라이프헬퍼** | 병원 근처 회복 숙소 예약 (lifehelper.co.kr) | 의료 인접 — 효능 주장 금지 |
| **넥스트헬퍼** | 숙소 운영 물품 공급 파트너십 | |
| **BGN** | 비영리 재단 — 순수익 10% 환원 | 매출 아님, 기부 지출 |
| **유니** | AI 화자 캐릭터 · 4채널 콘텐츠 · 홍보모델 | 톤·발행은 `uni` 스킬이 단일 출처 |

**브랜드 사실관계·법적 가드레일·계측 경로 전체는 `helper-universe` 스킬(`.claude/skills/helper-universe/SKILL.md`)에 있습니다.** 브랜드 관련 문구·기획·영업·재무 작업 전에 그 스킬을 읽으세요.

운영 체계와 부서별 실행 프롬프트: `docs/AI_COMPANY_OS.md`, `docs/prompts/company-os/`.
